...
function SkeletonBlock({ className }: { className?: string }) {
  return <div className={cn('bg-overlay/5 rounded animate-pulse', className)} />;
}
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      {Array.from({ length: 6 }).map((_, i) => (
        <div key={i} className="glass-light rounded-xl p-5 border border-overlay/5">
          ...
          <div className="flex items-center justify-end mt-4 pt-4 border-t border-overlay/5">
  ...
  return (
    <div className="space-y-2">
      {Array.from({ length: 8 }).map((_, i) => (
        <div
          key={i}
          className="flex items-center gap-4 p-4 rounded-xl bg-overlay/[0.02] border border-black/[0.03]"
        >
